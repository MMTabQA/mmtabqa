...
if DATASET == "fetaqa_MM_cleaned":
    ...
    for qid, response in tqdm(qid_to_response.items()):
        gold_ans = response['gold_ans']
        prompt = response["prompt"].split("Now, using the above examples as context, reason and answer the question given:")[-1]
    ...
    print(exact_match, "/", len(f1_scores))

    filename = '/home/suyash/final_repo/modelling/upper_bound/Results/gpt_evaluator_prompts_random.json'
